...
#pragma region IMPLEMENTATION
  template<class T, class = is_enum_t<T>>
  inline constexpr typename std::underlying_type<T>::type _underlying_value(T p_flag)
  {
    return static_cast<typename std::underlying_type<T>::type>(p_flag);
  }

  template<class T, class>
  inline constexpr T operator~(T p_flag)
  {
    return static_cast<T>(~_underlying_value(p_flag));
  }

  template<class T, class>
  inline constexpr T operator&(T p_flag_1, T p_flag_2)
  {
  ...
  template<class T, class>
  inline constexpr bool is_flag_set(T p_value, T p_flag)
  {
